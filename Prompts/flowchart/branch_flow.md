...
判断结果只能作为 edge.label，不能作为 node.text。
...
只有以下情况可以生成 label="返回" 的回边：

1. 原文明确出现：
   返回、回到、重新输入、重新上传、重新提交、重新支付、退回修改、补充材料。

2. 节点文本本身包含明确返工语义：
   重新上传、重新输入、重新选择、重新支付、退回修改、补充材料、提示登录失败、提示验证码错误。

3. 原文明确说明“流程返回到 X”，则该节点应连接回 X 对应的前置输入 / 提交 / 操作节点。

禁止行为：

1. 不要因为 condition 是“失败 / 不通过 / 无效”就自动返回。
2. 不要因为节点包含“提示”就自动返回。
3. 如果原文写的是“提示完成后，流程结束”，则该节点应连接到“流程结束”。
4. 如果原文写的是“提示完成后，系统继续执行 Y”，则该节点应连接到 Y，不是返回。
5. 如果中间存在新的判断节点，例如“是否重新支付 / 是否重试 / 是否补充材料”，必须保留该判断节点，不能直接用返回边代替。
